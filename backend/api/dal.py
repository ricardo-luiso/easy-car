...
async def crear_auto(db: AsyncSession, auto: schemas.AutoCreateRequest):
    ...
    return JSONResponse(content={"message": "ok", "id": nuevo_auto.id}, status_code=201)

async def obtener_autos(db: AsyncSession):
    result = await db.execute(select(models.Auto))
    return result.scalars().all()

async def obtener_auto_por_id(db: AsyncSession, auto_id: int):
    result = await db.execute(select(models.Auto).where(models.Auto.id == auto_id))
...
async def modificar_auto(db: AsyncSession, auto_id: int, auto: schemas.AutoCreateRequest):
    auto_actual = await db.get(models.Auto, auto_id)
    if not auto_actual:
        raise HTTPException(status_code=404, detail="Auto no encontrado")

    # Solo actualiza la imagen si hay una nueva
    data = auto.dict(exclude_unset=True)
    for key, value in data.items():
        if value is not None:
            setattr(auto_actual, key, value)

    await db.commit()
    await db.refresh(auto_actual)
