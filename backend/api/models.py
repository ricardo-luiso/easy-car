...
class Marca(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    nombre = Column(String, nullable=False)
    modelo = relationship("Modelo", back_populates="modelos")
class Color(Base):
    __tablename__ = "color"
    id = Column(Integer, primary_key=True, index=True)
    nombre = Column(String, nullable=False)
...
class Modelo(Base):
    ...
    nombre = Column(String, nullable=False)
    marca_id = Column(Integer, ForeignKey("marca.id"))
    marca = relationship("Marca", back_populates="marca", uselist=False)
...
class Venta(Base):
    ...
    estado_id = Column(Integer, ForeignKey("estado.id"))
    estado_id = Column(Integer, ForeignKey("auto.id"))
class Vendedor(Base):
